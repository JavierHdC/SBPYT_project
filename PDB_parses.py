...
class Featurizer():
    ...
    def __init__(self, atom_codes=None, atom_labels=None,
                 ...
                 smarts_labels=None, protein_properties = False,
                 path = '/home/binnu/Asad/dataset/new_db/featurized_pdb/'):
        ...
        ---New Additions---
        protein_properties: bool, required
            It is a parameter to whether include the properties dervied from
            amino acid level. It includes Secondary Structure (SS), Dihedral Angle
            (Phi, Psi), Relative Solvent Accesiblity (RSA)
        path: string, optional
            The path to the directory where pickled files of each PDB is stored of 
        ...
        # Secondary Structure, Phi, Psi, RSA
        self.take_protein = False
        if protein_properties:
            self.take_protein = True
            self.PATH = path
            self.FEATURE_NAMES += ['SS', 'Phi', 'Psi', 'RSA']

    def load_protein_proterties(self, PDBID):
        """It loads the protein properties from the pickled files of each PDB ID
        """
        with open(os.path.join(self.PATH, PDBID + ".dat"), "rb+") as f:
            prop = pickle.load(f)
        self.protein_property_dict = prop
    ...
    def get_features(self, molecule, PDB_ID, molcode=None):
        ...
        pdbID: string, required
            The PDB ID of currently working protein so the respective properties can
        ...
        features: np.ndarray, shape = (N, F)
            ...
            SMARTS patterns
        """
        if(self.take_protein):
            self.load_protein_proterties(PDB_ID)
        ...
        for i, atom in enumerate(molecule):
            ...
            if atom.atomicnum > 1:
                ...
                if(self.take_protein):
                    if(tuple(atom.coords) not in self.protein_property_dict.keys()):
                        print("")
                        protein_prop = [0, 0, 0, 0]
                    else:
                        protein_prop = list(self.protein_property_dict[tuple(atom.coords)])
